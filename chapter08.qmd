...
```
従属変数$y_i$が$0$か$1$の二値であるという観察可能な行動の結果を示すものとします。
独立変数$x_i$は、その選択に影響を与える要因を示す連続変数とします。
ここで$i$は観測値の番号を示し，全部で$n$のサンプルサイズがあるとします。
...
となり，教科書p.237の2つ目の式が成り立つことが分かります。これを反応確率と呼ぶようです。

$$
\Delta P(y_i = 1 \mid x_i) = \beta_1 \cdot \Delta x_i 
$$

これは$x_i$ が1単位増えると、$y_i = 1$ となる確率が $\beta_1$ だけ変化する，ということです。
線形確率モデルは，
