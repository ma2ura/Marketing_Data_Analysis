...
```{r code6-1, filename="code 6-1 Excelデータの読み込み"}
...
```{r code6-4, filename="code 6-4 変数間の関係の概略"}
#| message: false
firmdata19 |>
...
`summary()`関数で結果を表示するのが簡単ですが、見づらいので、ここでは`modelsummary`パッケージを使ってきれいに表示します。
...
近年は回帰分析の結果を適切に解釈するためにp値だけではなく、信頼区間も確認することが推奨されています。
Rでは`confint()`関数を使って信頼区間を計算します。
この`confint()`関数で計算される信頼区間はデフォルトで95%信頼区間となっています。
